  ...
  complex_msgs__msg__MultiStringTest__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    complex_msgs__msg__MultiStringTest * data =
      (complex_msgs__msg__MultiStringTest *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!complex_msgs__msg__MultiStringTest__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          complex_msgs__msg__MultiStringTest__fini(&data[i]);
        }
        free(data);
        return false;
      }
